...
const BlueprintCanvas = () => {
  ...
  useEffect(() => {
    ...
    const A = (o: number) => `rgba(78,125,140,${o})`;
    ...
    function draw() {
      ...
      function door(cx: number, cy: number, r: number, a0: number, a1: number, ccw: boolean) {
      ...
      ctx.lineWidth = 2;
      function wdw(x1: number, y1: number, x2: number, y2: number) {
        ctx.beginPath();
        ctx.moveTo(x1, y1);
        ctx.lineTo(x2, y2);
        ctx.stroke();
      ...
      function dim(x1: number, y1: number, x2: number, y2: number, label: string, lx: number, ly: number, angle: number | null) {
